...
import (
	...
	vslib "github.com/tomxucnxa/vehiclesharing/lib"
)
...
func main() {
	log.Println("Begin to start the chaincode VehicleSharing")

	var vsutil = new(vslib.VSUtil)
	log.Println(vsutil.GetTime())
	var err error = shim.Start(new(VehicleSharing))
	if err != nil {
		log.Println("Starting the chaincode VehicleSharing get failed.")
